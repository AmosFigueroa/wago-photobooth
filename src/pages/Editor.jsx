...
const Editor = () => {
  ...
  const SCRIPT_ID = "AKfycbyg1IZ8lTWCz3y-r-VS4E-s6fz9ug1rtu6id5w8uOd4eBmWtu_-VAEt8ZGTW408cfsu";
  ...
  }, [rawPhotos, layoutType, bgType, frameColor, selectedPattern, patternColor, stickers, navigate]); // Dependency lengkap

  const drawDecorations = (ctx, w, h) => {
    // Footer Background agar tulisan terbaca
    ctx.fillStyle = bgType === 'pattern' ? 'rgba(255,255,255,0.85)' : 'transparent';
                ...
                <div>
                    ...
                    <div className="grid grid-cols-6 gap-2">
                        <div className="relative aspect-square rounded-full border-2 overflow-hidden bg-gradient-to-tr from-pink-200 to-blue-200 cursor-pointer group">
                            <input type="color" value={frameColor} onChange={(e) => { setFrameColor(e.target.value); setBgType('color'); }} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10"/>
                            <div className="absolute inset-0 flex items-center justify-center text-gray-600 pointer-events-none group-hover:text-pink-600"><Pipette size={16}/></div>
                        </div>
                        {colors.map(c => (<button key={c} onClick={()=>{setFrameColor(c); setBgType('color');}} className={`aspect-square rounded-full border-2 shadow-sm ${bgType==='color' && frameColor===c ? 'border-pink-500 scale-110':''}`} style={{backgroundColor:c}}/>))}
                    </div>
                </div>
                        ...
                        <div className="grid grid-cols-3 gap-2">
                            {customAssets.bgs.map(bg => (
                                <button 
                                    ...
                                    className={`aspect-video rounded-lg border-2 overflow-hidden hover:border-pink-500 transition relative ${bgType === 'image' && selectedPattern === bg.url ? 'border-pink-500 ring-2 ring-pink-200' : 'border-gray-100'}`}
                                >
                ...
                <div>
                    ...
                    <div className="flex items-center gap-2 mb-2 bg-gray-50 p-2 rounded-lg border border-gray-100">
                        ...
                        <div className="flex flex-col">
                            <span className="text-xs text-gray-500 font-bold">Ganti Warna Pola</span>
                            <span className="text-[10px] text-gray-400">Klik lingkaran di kiri</span>
                        </div>
                    </div>
                    <div className="grid grid-cols-3 gap-2">
                        {patterns.map(p => (
                            <button 
                                key={p.id} 
                                onClick={() => { setSelectedPattern(p.id); setBgType('pattern'); }} 
                                className={`h-12 border rounded-lg bg-white text-xs font-bold text-gray-600 hover:bg-pink-50 hover:border-pink-300 transition ${bgType === 'pattern' && selectedPattern === p.id ? 'border-pink-500 bg-pink-50 text-pink-600' : ''}`}
                            >
                                {p.name}
                            </button>
                        ))}
          ...
          {activeTab === 'sticker' && (
            <div className="space-y-6">
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-4 gap-2">
                            {customAssets.stickers.map(s => (
                                <button key={s.url} onClick={()=>addSticker(s.url, 'image')} className="aspect-square p-1 border rounded hover:bg-gray-50 hover:border-pink-300 transition">
                ...
                <div className="grid grid-cols-5 gap-2">
                    {['❤️','⭐','🔥','✨','🌸','📸','🦋','👑','😎','🌈'].map(e => (<button key={e} onClick={()=>addSticker(e, 'emoji')} className="text-2xl hover:scale-125 transition p-2 bg-white rounded-lg border border-gray-100">{e}</button>))}
                </div>
                <button onClick={()=>setStickers([])} className="w-full py-3 border-2 border-red-100 text-red-500 rounded-xl font-bold text-sm hover:bg-red-50 mt-4">Hapus Semua Stiker</button>
            </div>
          )}
        </div>
