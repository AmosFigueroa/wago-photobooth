...
import { Camera, Download, RefreshCcw, Layers, Zap, Image as ImageIcon, CreditCard, X } from 'lucide-react';
...
const WagoPhotobooth = () => {
  ...
  const processStripCapture = async () => {
    ...
    for (let i = 0; i < images.length; i++) {
      ...
      ctx.drawImage(img, padding, (padding * (i + 1)) + (height * i), width, height);
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 p-4 font-sans text-white overflow-hidden">
      <header className="flex justify-between items-center mb-6 max-w-4xl mx-auto backdrop-blur-md bg-white/10 p-4 rounded-2xl shadow-xl border border-white/20">
        ...
        <button onClick={() => setIsGalleryOpen(!isGalleryOpen)} className="p-2 bg-white/20 rounded-full hover:bg-white/30 transition shadow-lg">
      ...
      <div className="max-w-4xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 relative group">
          <div className="relative rounded-3xl overflow-hidden shadow-2xl border-4 border-white/30 bg-black aspect-[4/3]">
            ...
            {finalImage ? (
              ...
              <video ref={videoRef} autoPlay playsInline className="w-full h-full object-cover transform -scale-x-100" style={{ filter: filter }} />
            )}
            {!finalImage && (
              <div className="absolute bottom-6 left-0 right-0 flex justify-center items-center gap-6">
                <button onClick={() => setMode(mode === 'single' ? 'strip' : 'single')} className="bg-black/50 p-3 rounded-full hover:bg-black/70 backdrop-blur text-xs font-bold text-white uppercase tracking-widest border border-white/20">
                  {mode === 'single' ? 'Single' : '4-Strip'}
                </button>
                <button onClick={handleCapture} className="w-20 h-20 rounded-full bg-white border-4 border-pink-500 shadow-[0_0_30px_rgba(236,72,153,0.6)] hover:scale-110 active:scale-95 transition duration-200 flex items-center justify-center">
          ...
          {finalImage && (
            <div className="flex gap-4 mt-4 justify-center animate-fade-in-up">
              <button onClick={() => setFinalImage(null)} className="flex items-center gap-2 px-6 py-3 bg-red-500/80 hover:bg-red-600 rounded-xl font-bold backdrop-blur shadow-lg transition">
                <RefreshCcw className="w-5 h-5" /> Retake
              </button>
              <button onClick={saveToGallery} className="flex items-center gap-2 px-6 py-3 bg-green-500/80 hover:bg-green-600 rounded-xl font-bold backdrop-blur shadow-lg transition">
        ...
        <div className="space-y-6">
          <div className="bg-white/10 backdrop-blur-md p-6 rounded-3xl border border-white/20 shadow-xl">
            <h3 className="text-lg font-bold mb-4 flex items-center gap-2"><Layers className="w-5 h-5 text-yellow-300" /> Filters</h3>
            <div className="grid grid-cols-2 gap-3">
              {filters.map((f) => (
                <button key={f.name} onClick={() => setFilter(f.value)} className={`p-3 rounded-xl text-sm font-medium transition-all duration-200 ${filter === f.value ? 'bg-pink-500 text-white shadow-lg scale-105 ring-2 ring-pink-300' : 'bg-white/20 hover:bg-white/30 text-white'}`}>
                  {f.name}
                </button>
              ))}
            </div>
          ...
          <div className="bg-gradient-to-br from-indigo-900/80 to-purple-900/80 backdrop-blur-md p-6 rounded-3xl border border-indigo-400/30 shadow-2xl relative overflow-hidden group">
            <h3 className="text-xl font-bold mb-2 flex items-center gap-2"><Zap className="w-6 h-6 text-yellow-400 fill-current" /> Premium</h3>
            <p className="text-sm text-indigo-200 mb-6">Unlock HD frames & cloud storage.</p>
            <button onClick={handlePayment} className="w-full py-4 bg-white text-indigo-900 rounded-xl font-bold text-lg hover:shadow-[0_0_20px_rgba(255,255,255,0.4)] transition-all flex items-center justify-center gap-2 group-hover:scale-[1.02]">
              <CreditCard className="w-5 h-5" /> Unlock Rp 25k
            </button>
            <p className="text-xs text-center mt-3 text-white/40">Secured by WAGO Payment</p>
          </div>
        </div>
      </div>
      ...
      {isGalleryOpen && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-lg flex items-center justify-center p-4">
          <div className="bg-white/10 border border-white/20 w-full max-w-4xl h-[80vh] rounded-3xl p-6 relative flex flex-col">
            <button onClick={() => setIsGalleryOpen(false)} className="absolute top-4 right-4 p-2 bg-red-500 rounded-full hover:bg-red-600 transition"><X className="w-6 h-6" /></button>
